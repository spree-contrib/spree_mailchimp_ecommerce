  ...
  class ImportExistingJob < ApplicationJob
    ...
    def upload_customers
      Spree::User.find_each do |u|
        begin
          Gibbon::Request.ecommerce.stores(ENV['MAILCHIMP_STORE_ID']).customers.create(body: u.mailchimp_decorated_user)
        rescue StandardError => error
          data = { msg: "user: #{u.id} error: #{error.message}" }
    ...
    def upload_products
      Spree::Product.includes(:variants).find_each do |p|
        begin
          ...
          data = { msg: "product: #{p.id} error: #{error.message}" }
          ::Errors::CustomNotifier.new.call(error, data)
        end
      end
    end
    ...
    def upload_carts
      Spree::Order.where.not(state: 'complete').includes(:line_items).find_each do |o|
        begin
          ...
          data = { msg: "order: #{o.id} error: #{error.message}" }
          ::Errors::CustomNotifier.new.call(error, data)
        end
      end
    end
